...
?>
<div id="group-{{ $id_name }}" class="form-group {!! $errors->has($error_name) ? 'has-error' : null !!}{{ !empty($special_symbol) ? ' with-special' : null }}">
    ...
    @if (!empty($special_symbol))
        @include('includes.special_symbols',['id_name'=>$id_name])
    @endif
